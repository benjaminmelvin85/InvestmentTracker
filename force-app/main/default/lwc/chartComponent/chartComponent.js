...
export default class ChartComponent extends LightningElement {
    ...
    handleChange(event) {
        ...
        console.log('my chartValue ' + this.chartValue);
        this.initializeChart();
    }
    // handleClick() {
    ...
    async initializeChart() {
        ...
        try {
            const apiKey;
            const apiUrl = `https://financialmodelingprep.com/api/v3/historical-price-full/${this.stockSymbol}?apikey=${apiKey}`;
            ...
            const stepSize = Math.ceil((max - min) / 20);
            const chartConfig = {
                type: 'line',
                data: {
